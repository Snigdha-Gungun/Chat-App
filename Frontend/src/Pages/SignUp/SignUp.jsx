  ...
  return (
   <div className='flex flex-col items-center justify-center min-w-96 mx-auto min-h-screen' style={{background: 'linear-gradient(-45deg, rgba(254,249,195,1) 0%, rgba(254,240,138,1) 50%, rgba(250,204,21,1) 100%)'}}>
      <div
        ...
        style={{ backgroundImage: `url(${psyduckBg})` }}
      ></div>
      <div className='relative z-10 w-full max-w-sm p-4 rounded-lg shadow-xl bg-gray-400 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-5'>
        <h1 className='text-3xl font-bold text-center text-gray-900'>SignUp{" "}
        ...
        <form onSubmit={handleSubmit}>
          <div>
            <label className="input input-bordered flex items-center gap-2 mt-5 bg-yellow-100">
              <svg
            ...
            <label className="input mt-5 bg-yellow-100 input-bordered flex items-center gap-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 16 16"
                fill="rgba(100,100,100,1)"
          ...
          <Link to='/login' className='text-sm font-semibold text-gray-900 hover:underline hover:text-yellow-700 mt-5 inline-block' >Already have an Account ?</Link>

          <button className="btn mt-2 btn-block text-white bg-yellow-600
          border-yellow-600 hover:bg-yellow-400 hover:text-yellow-700 hover:border-2 hover:border-yellow-700" >SignUp</button>
        </form>
      </div>
    </div>
  )
